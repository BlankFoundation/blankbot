...
async function updateRecordWithNewVoucher(recordId, voucher) {
  await database("Whitelist").update(
    recordId,
    {
      Voucher: voucher,
    },
...
const regenerateVoucher = async (interaction) => {
  if (interaction.member.roles.cache.some((role) => role.name === "Council")) {
    ...
    const discordUserNameRecords = await getDiscordUserNameRecords(
      discordUserName
    );
    if (discordUserNameRecords && discordUserNameRecords.length > 0) {
      try {
        const walletAddress =
          discordUserNameRecords[0]["fields"]["WalletAddress"];
      ...
      await interaction.reply({
        content:
          "Wallet address not yet added to whitelist for member " +
          discordUserName +
          ". Please ask them to run the /whitelist command to add themselves!",
        ephemeral: true,
      });
    }
  } else {
    await interaction.reply({
      content:
        "You must be a council member to regenerate a voucher for a user. Please message one of them to help you out.",
      ephemeral: true,
    });
  }
};
